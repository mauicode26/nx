        ...
        (event.descriptor as? JvmTestOperationDescriptor)?.className?.let { className ->
          tasks.entries
              ...
              ?.let { nxTaskId ->
                ...
                val methodName = event.descriptor.name
                when (event.result) {
                  is TestSuccessResult -> {
                    if (testTaskStatus[nxTaskId] != false) {
                      testTaskStatus[nxTaskId] = true
                    }
                    logger.info("✅ Test passed:  $nxTaskId $className.$methodName")
                  }
                  is TestFailureResult -> {
                    testTaskStatus[nxTaskId] = false
                    logger.warning("❌ Test failed: $nxTaskId $className.$methodName")
                  }
                  is TestSkippedResult -> {
                    if (testTaskStatus[nxTaskId] != false) {
                      testTaskStatus[nxTaskId] = true
                    }
                    logger.warning(" Test skipped: $nxTaskId $className.$methodName")
                  }
                  else -> {
                    if (testTaskStatus[nxTaskId] != false) {
                      testTaskStatus[nxTaskId] = true
                    ...
                    logger.warning(
                        "⚠️ Test finished with unknown result:  $nxTaskId $className.$methodName")
                  }
                }
              }
        }
